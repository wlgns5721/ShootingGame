...
class EnemyAct {
	...
	void enemyAttack(Game game) {
		...
		for (int i=0; i<game.enemyList.size(); i++) {
			...
			case 10:
				bossAttackPattern1(game,x,y,buf,buf.bulletSpeed);
				break;
			case 11:
				bossAttackPattern2(game,x,y,buf,buf.bulletSpeed);
				break;
			case 12:
				bossAttackPattern3(game,x,y,buf);
			}
			if (buf.enemy_X <= 1350 && buf.attackFlag ==true) {
				buf.attackFlag = false;
				...
				switch (buf.attackPattern) {
				case 1:
					attackPattern1(game,x,y,buf.bulletSpeed,buf.level);
					break;
				case 2:
					attackPattern2(game,x,y,(double)game.x,(double)game.y,buf.bulletSpeed);
					break;
				case 3:
					attackPattern3(game,x,y,buf,buf.bulletSpeed);
					break;
				case 4:
					attackPattern4(game,x,y,buf,buf.bulletSpeed);
					break;
				case 5:
					attackBombPattern2(game,x,y,buf,buf.bulletSpeed);
					break;
				case 6:
					attackBombPattern1(game,x,y,buf,buf.bulletSpeed);
					break;
	...
	void bomb(Game game) {
		for (int i=0; i<game.bombList.size(); i++) {
			...
			handleBoom(i,buf,buf.explodePattern,game);			
		}
	}
	...
	//��ä�÷� ���ÿ� �߻�
	void attackPattern1(Game game, double x, double y,int speed,int level) {
		int angle;
		switch (level) {
		case 1:
			angle = 30;
	...
	//���� �������� �߻�
	void attackPattern2(Game game, double x, double y, double Character_x,double Character_y, int speed) {
	...
	//��Ѹ��� ���
	void attackPattern3(Game game, double x, double y,Enemy enemy, int speed) {
		EnemyBullet enemyBullet = new EnemyBullet(x,y+25,speed,enemy.numberOfBullet*5-(int)enemy.fireCount*10+enemy.fireCount/8);
		game.enemyBulletList.add(enemyBullet);
	}
	
	//��ä�÷� ������� �߻�
	void attackPattern4(Game game, double x, double y,Enemy enemy, int speed) {
	...
	//������ ���� ���⿡ ���ߵ� ��ź
	void attackBombPattern1(Game game, double x, double y, Enemy enemy, int flag_speed) {
	...
	//����� ���� ��ź
	void attackBombPattern2(Game game, double x, double y, Enemy enemy, int flag_speed) {
		Bomb bomb = new Bomb(x,y,15,0);
	...
	void bossAttackPattern1(Game game, double x, double y, Enemy enemy, int flag_speed) {
		if (game.flowIndex==5400) 
			enemy.setAttackDelay(1000, 1000, 1);
		if (game.flowIndex>=5400 && game.flowIndex < 5600 && enemy.attackFlag==true) 
			attackBombPattern2(game,x,y*19/15,enemy,flag_speed);	
		
		else if (game.flowIndex>=5600 && game.flowIndex < 5800 && enemy.attackFlag==true) {
			game.createEnemy.setAttackPattern(2, 2, enemy);
			enemy.setAttackDelay(70, 500, 3);
			attackPattern2(game,x+50,y+20,game.x,game.y,15);
			attackPattern2(game,x+120,y,game.x,game.y,15);
			attackPattern2(game,x,y*19/15,game.x,game.y,15);
			attackPattern2(game,x+50,y+110,game.x,game.y,15);
			attackPattern2(game,x+150,y+130,game.x,game.y,15);
		}
	...
	void bossAttackPattern2(Game game, double x, double y, Enemy enemy, int flag_speed) {
		if (game.flowIndex==5800) {
			enemy.delay = 0;
			enemy.setAttackDelay(50, 300, 8);
		}
		if (game.flowIndex>=5800 && game.flowIndex < 5900 && enemy.attackFlag==true) { 
			attackPattern1(game,x,y*19/15,10,3);
			attackPattern1(game,x,y*17/15,10,3);
		}
		
		else if (game.flowIndex>=5900 && game.flowIndex < 6150 && enemy.attackFlag==true) {
			game.createEnemy.setAttackPattern(2, 2, enemy);
			...
			enemy.setAttackDelay(50, 300, 8);
		else if (game.flowIndex >=6150 && game.flowIndex <6250 && enemy.attackFlag==true) {
			attackPattern1(game,x,y*19/15,10,3);
			attackPattern1(game,x,y*17/15,10,3);
		}
		
		else if (game.flowIndex == 6250) {
			attackBombPattern1(game,x,y,enemy,10);
		}
	...
	void bossAttackPattern3(Game game, double x, double y, Enemy enemy) {
		...
		if (game.flowIndex%100==0 && (game.flowIndex<8600 || game.flowIndex>8800) &&( game.flowIndex<9500 || game.flowIndex>9700)) {
			if (game.pase_2)
			attackPattern1(game,x+233,y+140,10,3);
		}
		if (game.flowIndex==8400)
			enemy.setAttackDelay(10, 300, 36);
		else if (game.flowIndex>8400 && game.flowIndex<8600 && enemy.attackFlag==true) {
			attackPattern2(game,x+30,y+10,game.x,game.y,25);
			attackPattern2(game,x+20,y+140,game.x,game.y,25);
			attackPattern2(game,x+30,y+270,game.x,game.y,25);
		}
		else if (game.flowIndex==8600)
			game.enemyAct.bossIndex=60;
		else if (game.flowIndex>=8600 && game.flowIndex < 8800 && game.flowIndex%2==0) {
			EnemyBullet enemyBullet = new EnemyBullet(x+20,y+140,10,bossIndex);
			game.enemyBulletList.add(enemyBullet);
			bossIndex-=1.4;
		}
		else if (game.flowIndex==8900) 
			attackBombPattern1(game,x+30,y+10,enemy,10); 
		else if (game.flowIndex==8910)
			attackBombPattern1(game,x+30,y+270,enemy,10);
			...
			EnemyBullet enemyBullet = new EnemyBullet(x+30,y+10,10,60-bossIndex);
			game.enemyBulletList.add(enemyBullet);
			EnemyBullet enemyBullet2 = new EnemyBullet(x+30,y+270,10,-60+bossIndex);
			game.enemyBulletList.add(enemyBullet2);
			bossIndex+=10;
		}
		else if (game.flowIndex==9320)
			attackBombPattern1(game,x+20,y+140,enemy,10);
		else if (game.flowIndex == 9500)
			enemy.setAttackDelay(50, 300, 8);
		else if (game.flowIndex >=9500 && game.flowIndex < 9650 && game.flowIndex%10==0) {
			attackPattern1(game,x+30,y+10,8,3);
			attackPattern1(game,x+20,y+140,8,3);
			attackPattern1(game,x+30,y+270,8,3);
		}
		else if (game.flowIndex==9750)
			game.flowIndex=8399;
	}
	void handleBoom(int i, Bomb buf, int rangeType, Game game) {
		buf.count--;
