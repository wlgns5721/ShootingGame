...
class Shoot {
	public int skillFireDelay = 0;
	public int skillLaunchAmount = 0;
	...
	boolean collisionCheck(int bullet_X,int bullet_Y,int width,int height, int damage,Game game) {
		...
		for (int i=0; i<game.enemyList.size(); i++) {
			...
			if (intersects(enemy,p)) {
				enemy.HP-=damage;
				game.grap.drawImage((Image)game.attackedEnemyImageList.elementAt(enemy.imageNumber),(int)enemy.enemy_X,(int)enemy.enemy_Y,game);
				if (enemy.HP<=0) {
					game.score+=enemy.score;
					game.maxEnemy--;
					game.playSound("..\\rsc\\explode"+enemy.attackPattern%8+".wav");
					...
					game.itemHandling.createItem(enemy.enemy_X+enemy.width*0.2, enemy.enemy_Y+enemy.height*2/5, (int)(Math.random()*2)+1, enemy.maxHP,game);					
				}
				return true;
			}
		}
	...
	void skill(Game game) {
		if (skillFireDelay==0) {
			...
			skillLaunchAmount++;
			skillFireDelay = game.skill.delay;
		}
		skillFireDelay--;
		if (skillLaunchAmount >= game.skill.launchAmount) {
